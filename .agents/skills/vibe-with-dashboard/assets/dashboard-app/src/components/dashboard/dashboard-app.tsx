            ...
            >
              ...
              <div className="mt-3 grid gap-2">
                {milestone.cards.slice(0, 4).map((card) => {
                  ...
                  const isActiveCard = cardStatus === "doing";
                  return (
                    <div
                      key={card.id}
                      data-active-card={isActiveCard ? "true" : "false"}
                      data-card-plan-status={card.status}
                      className={`flex items-center gap-2 rounded-md border border-l-4 border-border px-2 py-1.5 ${planCardAccentClass[cardStatus]} ${
                        isActiveCard ? "plan-card-active" : ""
                      }`}
                    >
                      <CircleDot
                        className={`size-3 ${planCardDotClass[cardStatus]} ${
                          isActiveCard ? "current-work-beacon" : ""
                        }`}
                      />
                      <span className="min-w-0 flex-1 truncate text-[11px]">
                        {localizedText(card, locale, "title", card.title)}
                      </span>
                      <StatusBadge messages={messages} status={cardStatus} />
                    </div>
                  );
                })}
              </div>
...
function CurrentWorkVisual({
  ...
  const isActivelyWorking = focusCard?.status === "doing";

  return (
    <section
      data-active-work={isActivelyWorking ? "true" : "false"}
      className={`rounded-md border border-border bg-muted/10 p-3 ${
        isActivelyWorking ? "current-work-active" : ""
      }`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
      ...
      <div className="mt-4">
        ...
        <div className="relative h-2 rounded-full bg-muted" data-testid="work-progress-track">
          <div
            ...
            className={`work-progress-fill absolute left-0 top-0 h-2 rounded-full ${
      ...
      <div
        ...
        data-active-focus={isActivelyWorking ? "true" : "false"}
        className={`relative mt-4 overflow-hidden rounded-md border border-border bg-background p-3 ${
          isActivelyWorking ? "current-focus-card" : ""
        }`}
      >
        <div className="flex items-center justify-between gap-3">
        ...
        <div className="mt-1 flex min-w-0 items-center gap-2">
          {isActivelyWorking && (
            <span className="current-work-beacon size-2 rounded-full bg-status-running" />
          )}
  ...
  return (
    <article
      ...
      className={`rounded-md border border-border bg-background p-2 shadow-sm ${
        card.status === "doing" ? "kanban-card-active" : ""
      } ${
...
function RubberDuck({
  ...
  function handleDuckClick() {
    ...
    setChipsVisible((visible) => !visible);
    setBurstKey((value) => value + 1);
  }
      ...
      {chipsVisible && (
        <div className="flex max-w-[min(22rem,calc(100vw-2rem))] flex-col items-end gap-2">
          {visibleSuggestions.length > 0 ? (
            ...
            <div
              key={`duck-idle-${burstKey}`}
              data-testid="duck-idle-chip"
              className="duck-chip rounded-full border border-border bg-popover px-3 py-2 text-xs font-medium text-muted-foreground shadow-lg"
            >
              {messages.duckIdle}
            </div>
          )}
    ...
    <Tabs defaultValue="repo" className="flex min-h-0 flex-1 flex-col">
      ...
      <ScrollArea className="min-h-0 flex-1">
        ...
        <TabsContent value="design" className="m-0 p-4">
          <DesignPanel messages={messages} snapshot={snapshot} />
        </TabsContent>
...
function DesignPanel({
  ...
  const tokenFiles = snapshot.designSystem.files;
  const notes = snapshot.designSystem.notes;
  const hasDesignSystem =
    snapshot.designTokens.length > 0 || tokenFiles.length > 0 || notes.length > 0;

  if (!hasDesignSystem) {
    return (
      <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
        {messages.noProjectDesignSystem}
      </div>
    );
  }
          ...
          {tokenFiles.map((file) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge variant="outline">{file.tokenCount}</Badge>
              </div>
            </article>
          ))}
        </section>
        ...
        <section className="space-y-2">
          ...
          {notes.map((note) => (
            <article
              ...
              className="rounded-md border border-border bg-muted/10 p-3"
            >
              <h4 className="font-mono text-xs font-semibold">{note.path}</h4>
