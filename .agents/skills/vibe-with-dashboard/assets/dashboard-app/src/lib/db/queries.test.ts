...
describe("vibe dashboard state", () => {
  ...
  it("does not mutate archived board rows through active-board update helpers", async () => {
    upsertPlan({
      task: "Scoped archive updates",
      replace: true,
      cards: [{ title: "Scoped card", status: "ready" }],
    });
    replaceDuckSuggestions({
      suggestions: [{ keyword: "Scope", title: "Keep active scoped" }],
    });
    const planned = await getDashboardSnapshot();
    const archivedCardId = planned.cards[0].id;
    const archivedGoalId = planned.goals[0].id;
    const archivedMilestoneId = planned.goals[0].milestones[0].id;
    const archivedSuggestionId = planned.duckSuggestions[0].id;

    updateCard(archivedCardId, { status: "done" });
    addActivity({
      phase: "result",
      title: "Done",
      message: "Archive the scoped board.",
      task: "scoped",
    });

    const emptyActive = await getDashboardSnapshot();
    const activeActivityCount = emptyActive.activityEntries.length;
    expect(updateCard(archivedCardId, { status: "review" })).toMatchObject({
      updated: false,
    });
    expect(updateGoal(archivedGoalId, { status: "paused" })).toMatchObject({
      updated: false,
    });
    expect(updateMilestone(archivedMilestoneId, { status: "planned" })).toMatchObject({
      updated: false,
    });
    expect(markDuckSuggestionRead(archivedSuggestionId)).toMatchObject({
      updated: false,
    });

    const snapshot = await getDashboardSnapshot();
    expect(snapshot.activityEntries).toHaveLength(activeActivityCount);
    const archivedSnapshot = snapshot.archives[0].snapshot as {
      cards: Array<{ status: string }>;
    };
    expect(archivedSnapshot.cards[0].status).toBe("done");
  });
  ...
  it("does not seed dashboard app design tokens as project tokens", async () => {
    const snapshot = await getDashboardSnapshot();
    expect(snapshot.designTokens.map((token) => token.name)).not.toContain("--radius");
    expect(snapshot.designSystem.tokens).toHaveLength(0);
  });

  it("detects design tokens from the target project root only", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-design-"));
    fs.mkdirSync(path.join(tempRoot, "src"), { recursive: true });
    fs.writeFileSync(
      path.join(tempRoot, "src", "theme.css"),
      ":root { --brand-primary: oklch(0.7 0.18 210); }\n"
    );
    fs.writeFileSync(
      path.join(tempRoot, "design-tokens.json"),
    ...
    try {
      const snapshot = await getDashboardSnapshot();
      expect(snapshot.designTokens.map((token) => token.name)).toEqual(
        expect.arrayContaining(["--brand-primary", "color.accent"])
      );
      expect(snapshot.designTokens.map((token) => token.name)).not.toContain(
        "--status-ready"
      );
      expect(snapshot.designSystem.files.map((file) => file.path)).toEqual(
        expect.arrayContaining(["src/theme.css", "design-tokens.json"])
      );
    } finally {
...
describe("installer CLI", () => {
  ...
  it("posts explicit suggestion clears through the CLI", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-dashboard-clear-"));
    let payload = "";
    const server = http.createServer((request, response) => {
      if (request.url === "/api/health") {
        response.writeHead(200, { "content-type": "application/json" });
    ...
    const result = await new Promise<{ status: number | null; stderr: string }>(
      (resolve) => {
        const child = spawn(
          process.execPath,
          ["bin/vibe-with-dashboard.js", "suggest", "--project", tempRoot, "--clear"],
          {
            cwd: process.cwd(),
            windowsHide: true,
            env: {
    ...
    expect(result.status).toBe(0);
    expect(JSON.parse(payload)).toMatchObject({ suggestions: [] });
    fs.rmSync(tempRoot, { recursive: true, force: true });
  });
  ...
  it("refuses to post to a dashboard serving a different project root", async () => {
    const projectA = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-project-a-"));
    const projectB = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-project-b-"));
    const server = http.createServer((request, response) => {
      if (request.url === "/api/health") {
        response.writeHead(200, { "content-type": "application/json" });
        response.end(
    ...
    const result = await new Promise<{ status: number | null; stderr: string }>(
      (resolve) => {
        const child = spawn(
          process.execPath,
          ["bin/vibe-with-dashboard.js", "snapshot", "--project", projectA],
          {
            cwd: process.cwd(),
            windowsHide: true,
            env: {
    ...
    expect(result.stderr).toContain("dashboard target mismatch");
  });
});
