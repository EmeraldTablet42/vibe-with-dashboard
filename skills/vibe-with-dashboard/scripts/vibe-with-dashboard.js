...
const DASHBOARD_APP_DIR = path.join("assets", "dashboard-app");
const SKILL_CLI = path.join("scripts", "vibe-with-dashboard.js");
...
function isAppRoot(candidate) {
  return (
    ...
    exists(path.join(candidate, "scripts", "launcher.ts")) &&
    exists(path.join(candidate, "src"))
  );
}

function skillAppRoot(skillRoot) {
  return path.join(skillRoot, DASHBOARD_APP_DIR);
}

function isSkillRoot(candidate) {
  return (
    exists(path.join(candidate, "SKILL.md")) &&
    exists(path.join(candidate, SKILL_CLI)) &&
    exists(path.join(candidate, "scripts", "dashboard-ensure.cjs")) &&
    isAppRoot(skillAppRoot(candidate))
  );
}

function isLegacyRuntimeRoot(candidate) {
  return (
    exists(path.join(candidate, "SKILL.md")) &&
    exists(path.join(candidate, "package.json")) &&
...
function resolveAppRoot(projectRoot) {
  ...
  if (isSkillRoot(currentRoot)) return skillAppRoot(currentRoot);
  if (isAppRoot(path.join(currentRoot, DASHBOARD_APP_DIR))) {
    return path.join(currentRoot, DASHBOARD_APP_DIR);
  }
  if (isLegacyRuntimeRoot(currentRoot)) return currentRoot;

  const projectSkillRoot = skillRuntimePath(projectRoot);
  if (isSkillRoot(projectSkillRoot)) return skillAppRoot(projectSkillRoot);
  if (isLegacyRuntimeRoot(projectSkillRoot)) return projectSkillRoot;
...
function resolveSkillSourceRoot(sourceRoot) {
  if (isSkillRoot(sourceRoot)) return sourceRoot;
  const nestedSkill = path.join(sourceRoot, "skills", SKILL_NAME);
  if (isSkillRoot(nestedSkill) || exists(path.join(nestedSkill, "SKILL.md"))) {
    return nestedSkill;
  }
...
function copySkill(sourceRoot, targetRoot, dryRun) {
  ...
  log(
    `${dryRun ? "would install" : "install"} project skill runtime at ${target} (app: ${path.join(target, DASHBOARD_APP_DIR)})`
  );
...
function installProjectRules(projectRoot, dryRun) {
  const localCli = "node .agents/skills/vibe-with-dashboard/scripts/vibe-with-dashboard.js";
  const body = [
    "# Vibe with Dashboard",
    "",
...
function runNodeScript(scriptName, projectRoot, extraArgs = []) {
  const appRoot = resolveAppRoot(projectRoot);
  const currentRoot = packageRoot();
  const candidates = [
    path.join(appRoot, "scripts", scriptName),
    path.join(currentRoot, "scripts", scriptName),
    path.join(skillRuntimePath(projectRoot), "scripts", scriptName),
  ];
  const scriptPath = candidates.find(exists);
  if (!scriptPath) {
...
function normalizeHealthPath(filePath = "") {
  if (!filePath) return "";
  return path.resolve(String(filePath)).toLowerCase();
}

async function readDashboardHealth(baseUrl) {
  return JSON.parse(await getJson(`${baseUrl}/api/health`));
}

async function assertDashboardTarget(baseUrl, projectRoot, appRoot) {
  const health = await readDashboardHealth(baseUrl);
  if (health.appId !== "vibe-with-dashboard") {
    throw new Error(
      `dashboard target mismatch: unexpected appId ${health.appId || "(missing)"}`
    );
  }

  const expectedProject = normalizeHealthPath(projectRoot);
  const actualProject = normalizeHealthPath(health.projectRoot);
  if (!actualProject || actualProject !== expectedProject) {
    throw new Error(
      `dashboard target mismatch: expected projectRoot ${projectRoot}, got ${health.projectRoot || "(missing)"}`
    );
  }

  const expectedApp = normalizeHealthPath(appRoot);
  const actualApp = normalizeHealthPath(health.appRoot);
  if (actualApp && expectedApp && actualApp !== expectedApp) {
    throw new Error(
      `dashboard runtime mismatch: expected appRoot ${appRoot}, got ${health.appRoot}`
    );
  }
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (command === "smoke") {
    runNodeScript("live-dashboard-smoke.cjs", projectRoot, rest);
    return;
  }
  ...
  if (command === "snapshot") {
    await assertDashboardTarget(baseUrl, projectRoot, appRoot);
