...
export function getDatabasePath() {
  ...
  const requested = path.resolve(
    process.env.DASHBOARD_DB_PATH ?? path.join(dashboardDir, "dashboard.sqlite")
  );
  const allowedDir = path.resolve(dashboardDir);
  if (
    requested !== allowedDir &&
    !requested.startsWith(`${allowedDir}${path.sep}`)
  ) {
    throw new Error(
      `DASHBOARD_DB_PATH must stay inside the target project .dashboard directory: ${requested}`
    );
  }
  return requested;
}
...
function resetLegacySchemaIfNeeded() {
  ...
  const tableRows = getUserTables();
  if (tableRows.length > 0) {
    throw new Error(
      `Refusing to initialize unknown SQLite database at ${getDatabasePath()}; set DASHBOARD_DB_PATH to a Vibe with Dashboard database or remove the file first.`
    );
  }
...
export function initializeDatabase() {
  ...
  ensureColumn("duck_suggestions", "summary", "summary TEXT NOT NULL DEFAULT ''");
  ensureColumn("duck_suggestions", "detail", "detail TEXT NOT NULL DEFAULT ''");
  ensureColumn(
    "duck_suggestions",
    "action_prompt",
    "action_prompt TEXT NOT NULL DEFAULT ''"
  );
  ensureColumn("duck_suggestions", "priority", "priority TEXT NOT NULL DEFAULT 'medium'");
  ensureColumn("duck_suggestions", "source", "source TEXT NOT NULL DEFAULT 'agent'");
  ensureColumn(
    "duck_suggestions",
    "translations_json",
    "translations_json TEXT NOT NULL DEFAULT '{}'"
  );
  ensureColumn("duck_suggestions", "read_at", "read_at TEXT");
  ensureColumn(
    "duck_suggestions",
    "updated_at",
    "updated_at TEXT NOT NULL DEFAULT (datetime('now'))"
