...
7. Move exactly one matching Work Card to `doing` before each work unit, then to `done` or `review` when that unit finishes.
8. Seed Rubber Duck suggestions when useful, or explicitly clear them when there is no advice to show.
9. Run the live dashboard smoke after CLI updates when checking that the user's open dashboard matches the agent state.
10. Provide Plan/Kanban item translations for the user's native locale and `en` when the target locale is known.
...
| Current state | Overall progress, animated active focus card, and current activity. |
